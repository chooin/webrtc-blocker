...
不用打开 popup 就能看出当前标签页有没有在被保护：

| 工具栏 | 含义 |
|---|---|
| 彩色图标，角标为空 | 正在拦，本页还没拦到东西 |
| 彩色图标 + 数字 | 正在拦，数字是本页已拦截次数 |
| **灰**图标 + `OFF` | 总开关关闭 |
| **灰**图标 + `—` | 本站在白名单里，你主动放行了 |
| **灰**图标，角标为空 | `chrome://` 这类页面，扩展在那里本来就注入不进去 |

读法只有一条：**灰图标 = 这个标签页没有在被保护**，角标文字说明是哪一种原因。
反过来，角标非空一律意味着「有事」——正常工作时它是空的，不在工具栏上持续喊话。

有一种情况工具栏认不出来：**改完设置但页面还没重新加载**。补丁只在文档加载时进入页面，
所以那一刻设置说「该拦」而页面其实没被拦。工具栏的判定只看设置，识别不了它；
打开 popup 才会看到「需重新加载」——popup 会实测当前页面。
...
7. **不刷新页面**，把总开关重新打开，然后打开 popup：站点状态应显示「需重新加载」并给出重新加载按钮，**不能**显示「已拦截」——补丁只在文档加载时进入页面，此刻它确实没在拦。点重新加载后状态应变成「已拦截」。**顺带看一眼这条提示的排版**：说明文字与「重新加载」按钮应同处一行、按钮贴右且不越出卡片——popup 的排版没有任何自动化覆盖，只有这一步能发现它被压垮。
...
13. 关闭总开关，确认**每个**标签页的工具栏图标都变灰、角标显示 `OFF`；重新打开后恢复彩色。
14. 把当前站点加入白名单，确认该标签页角标变成 `—` 而其它标签页不受影响。
15. 切到一个 `chrome://` 页面，确认图标是灰的且角标为空——那里写 `OFF` 会被误读成「是我关的开关」。
